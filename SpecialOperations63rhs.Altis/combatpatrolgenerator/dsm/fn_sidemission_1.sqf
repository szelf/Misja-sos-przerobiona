...
private["_missionPos", "_mrkPos", "_barGate", "_tower", "_mrkPatrol", "_mrk"];
...
while {((count _roads) < 1) and (_counter < 100)} do {
	_ambush = [_patrolAreaPos, (1500*CPG_sideMissions_radiusMulti), (2500*CPG_sideMissions_radiusMulti), 5, 0, 1000, 0, CPG_blacklist, CPG_backupPos] call BIS_fnc_findSafePos;
...
if ((count _roads) >= 1) then {
	...
	[leader _grp, _mrkPatrol, "ONROAD", "random", "spawned"] execVM "scripts\UPSMON.sqf";
