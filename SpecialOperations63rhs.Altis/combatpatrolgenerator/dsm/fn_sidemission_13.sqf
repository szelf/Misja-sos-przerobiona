...
private["_missionPos", "_grpPos", "_overwatchPos", "_mrkPos", "_mrkPatrol", "_mrk", "_counter", "_aatank", "_quadGroup"];
...
if (_counter < 10000) then {
	...
	_aatank = createVehicle ["rhs_zsu234_aa", _missionPos, [], 0, "NONE"];
	_aatank setVehicleLock "LOCKED";
	_aatank setDir (random 360);

	// create group
	_groupTank = createGroup east;
	_ai = _groupTank createUnit ["rhs_vdv_driver_armored", _missionPos, [], 0, "NONE"];
	_ai moveInDriver _aatank;	
	_ai = _groupTank createUnit ["rhs_vdv_armoredcrew", _missionPos, [], 0, "NONE"];
	...
	if ((random 100) < 50) then{
		...
		for [{_i = 0},{_i < 3},{_i = _i + 1}] do {
			_quad  = createVehicle ["rhs_uaz_open_vdv", _missionPos, [], 0, "NONE"];
			_ai = _quadGroup createUnit ["rhs_vdv_rifleman", _missionPos, [], 0, "NONE"];
			_ai moveInAny _quad;
			_ai = _quadGroup createUnit ["rhs_vdv_grenadier", _missionPos, [], 0, "NONE"];
			_ai moveInAny _quad;
		};
		
		_mrkPatrol = [_missionPos, [500, 500]] call CPG_fnc_createPatrolMarker;
