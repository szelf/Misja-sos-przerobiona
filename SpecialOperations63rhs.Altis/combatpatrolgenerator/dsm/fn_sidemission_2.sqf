...
private["_missionPos", "_grpPos", "_overwatchPos", "_mrkPos", "_mrkPatrol", "_mrk", "_fob"];
...
while {(_equal1 || _equal2) && _counter < 10000} do {
	_missionPos = [_patrolAreaPos, (1500*CPG_sideMissions_radiusMulti), (2500*CPG_sideMissions_radiusMulti), 4, 0, 0, 0, CPG_blacklist, CPG_backupPos] call BIS_fnc_findSafePos;
...
if (_counter < 10000) then {
	...
	if(random(100) < 50) then {
		...
		_grp = [_missionPos, east, _grpCfg, [], [], [0.45,0.9], [], [], (random 360)] call BIS_fnc_spawnGroup;
		_mrkPatrol = [_missionPos, [100, 100]] call CPG_fnc_createPatrolMarker;
		[leader _grp, _mrkPatrol, "fortify", "spawned"] execVM "scripts\UPSMON.sqf";
