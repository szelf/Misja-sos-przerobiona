...
private["_missionPos", "_grpPos", "_overwatchPos", "_mrkPos", "_mrkPatrol", "_mrk"];
...
while { (_equal1 || _equal2) && _counter < 10000} do {
	_missionPos = [_patrolAreaPos, (1500*CPG_sideMissions_radiusMulti), (2500*CPG_sideMissions_radiusMulti), 10, 0, 0, 0, CPG_blacklist, CPG_backupPos] call BIS_fnc_findSafePos;
...
if (_counter < 10000) then {
	...
	_leader = _grp createUnit ["rhs_msv_junior_sergeant", _grpPos, [], 0, "CAN_COLLIDE"];
