...
switch (floor(random 10)) do {
	case 0:	{
		...
		_pos = [_patrolArea_position, 0, (2500*CPG_patrolArea_radiusMulti), 4, 0, 1000, 0, CPG_blacklist, CPG_backupPos] call BIS_fnc_findSafePos;
	...
	case 1:	{
		...
		_pos = [_patrolArea_position, 0, (2500*CPG_patrolArea_radiusMulti), 4, 0, 5, 0, CPG_blacklist, CPG_backupPos] call BIS_fnc_findSafePos;
		
		// create tower
		_tower = createVehicle ["Land_Cargo_Patrol_V1_F", _pos, [], 0, "CAN_COLLIDE"];
		_group_0 = createGroup east;
		_ai = _group_0 createUnit ["rhs_vdv_flora_grenadier", _pos, [], 0, "CAN_COLLIDE"];
		_ai setPos (_tower buildingPos 1);
		_group_1 = createGroup east;
		_ai = _group_1 createUnit ["rhs_vdv_flora_rifleman", _pos, [], 0, "CAN_COLLIDE"];
	...
	case 2:	{
		...
		_pos = [_patrolArea_position, 0, (2000*CPG_patrolArea_radiusMulti), 3, 0, 1000, 0, CPG_blacklist, CPG_backupPos] call BIS_fnc_findSafePos;
		...
		_mrkSize	= [[150, 150], [200, 200], [250, 250]] call BIS_fnc_selectRandom;
		_mrkPatrol	= [_pos, _mrkSize] call CPG_fnc_createPatrolMarker;
		[_pos, 50 * CPG_vehicleProb, _grp, "ATVs", true] call CPG_fnc_spawnVehicles;	// 50% chance to spawn armed ATV(s) for this group
	...
	case 3:	{
		...
		[_fortPos, 50 * CPG_vehicleProb, _grp, "ATVs", true] call CPG_fnc_spawnVehicles;	// 50% chance to spawn armed ATV(s) for this group
		CPG_createdEnemies = CPG_createdEnemies + (count units _grp);
		[leader _grp, _mrkPatrol, "nofollow", "fortify", "noveh", "spawned"] execVM "scripts\UPSMON.sqf";
	...
	case 5:	{
		...
		if (floor(random 100) < (35 * CPG_vehicleProb)) then {
			_pos = [_patrolArea_position, 0, (2000*CPG_patrolArea_radiusMulti), 7, 0, 0.5, 0, CPG_blacklist, CPG_backupPos] call BIS_fnc_findSafePos;
	...
	case 6:	{
		...
		if ((random 100) < (35 * CPG_vehicleProb)) then {
			...
			_apc = createVehicle ["rhs_btr80_vdv", _pos, [], 0, "CAN_COLLIDE"];
			_apc setVehicleLock "LOCKED";			
			_ai = _grp createUnit ["rhs_vdv_armoredcrew", _pos, [], 0, "CAN_COLLIDE"];
			_ai assignAsGunner _apc;
			_ai moveInGunner _apc;
			_ai = _grp createUnit ["rhs_vdv_driver_armored", _pos, [], 0, "CAN_COLLIDE"];
			...
			if ((random 100) < 30) then	{
				...
				_veh = createVehicle ["rhs_bmp1k_vdv", _vehPos, [], 0, "CAN_COLLIDE"];
				_ai = _grp createUnit ["rhs_vdv_armoredcrew", _vehPos, [], 0, "CAN_COLLIDE"];
				_ai assignAsGunner _veh;
				_ai moveInGunner _veh;
				_ai = _grp createUnit ["rhs_vdv_driver_armored", _vehPos, [], 0, "CAN_COLLIDE"];
	...
	case 7:	{
		...
		if (floor(random 100) < 35) then {
			...
			while {((count _roads) < 1) and (_tmpnr < 100)} do {
				_pos = [_patrolArea_position, (1500*CPG_patrolArea_radiusMulti), (2500*CPG_patrolArea_radiusMulti), 5, 0, 1000, 0, CPG_blacklist, CPG_backupPos] call BIS_fnc_findSafePos;
	...
	case 8:	{
		...
		if ((random 100) < 10) then {
			...
			_ai = _grp createUnit ["rhs_vdv_flora_marksman", _pos, [], 0, "CAN_COLLIDE"];
			_ai = _grp createUnit ["rhs_vdv_flora_marksman", _pos, [], 0, "CAN_COLLIDE"];
			CPG_createdEnemies = CPG_createdEnemies + (count units _grp);
			_mrkSize	= [[300, 300], [400, 400], [500, 500]] call BIS_fnc_selectRandom;
			_mrkPatrol	= [_pos, _mrkSize] call CPG_fnc_createPatrolMarker;
	...
	case 9:	{
		...
		if (CPG_mortars == 1) then {
			if ((random 100) < 10) then {
				...
				_grp = createGroup east;
				_gunner = _grp createUnit ["rhs_vdv_flora_rifleman", _pos, [], 0, "CAN_COLLIDE"];
				_gunner moveInGunner _mortar;
				_ai = _grp createUnit ["rhs_vdv_flora_junior_sergeant", _pos, [], 0, "CAN_COLLIDE"];
				_ai = _grp createUnit ["rhs_vdv_flora_grenadier", _pos, [], 0, "CAN_COLLIDE"];
				_ai = _grp createUnit ["rhs_vdv_flora_medic", _pos, [], 0, "CAN_COLLIDE"];
				CPG_createdEnemies = CPG_createdEnemies + (count units _grp);
				_mrkSize	= [[150, 150], [200, 200], [250, 250]] call BIS_fnc_selectRandom;
				_mrkPatrol	= [_pos, _mrkSize] call CPG_fnc_createPatrolMarker;
				...
				_behaviour = [
					["spawned"],
					["nomove", "spawned"]] call BIS_fnc_selectRandom;
				([leader _grp, _mrkPatrol] + _behaviour) execVM "scripts\UPSMON.sqf";
