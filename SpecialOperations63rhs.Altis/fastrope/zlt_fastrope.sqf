...
zlt_fnc_tossropes = {
	...
	_heli spawn {
		...
		_heli = _this;
		while {alive _heli and count (_heli getvariable ["zlt_ropes", []]) != 0 and abs (speed _heli) < MAX_SPEED_ROPES_AVAIL and ((getPos _heli select 2) >= 5) and ((getPos _heli select 2) <= 30)} do {
			sleep 0.3;
		};
...
zlt_fnc_ropes_cond = {
	_veh = vehicle player;
	_flag = (_veh != player) and {(not zlt_mutexAction)} and {count (_veh getvariable ["zlt_ropes", []]) == 0} and { (typeof _veh) in zlt_rope_helis } and {alive player and alive _veh and (abs (speed _veh) < MAX_SPEED_ROPES_AVAIL ) } and { (getPos _veh select 2) >= 5} and { (getPos _veh select 2) <= 30};
	_flag;
