...
UPSMON_surrended = {
	...
	//_npc action ["DropWeapon","GroundWeaponHolder" createVehicle getposATL _npc,primaryweapon _npc];
	//_secondweap = secondaryWeapon _npc;
	//_handweap = handgunWeapon _npc;
	//If (_secondweap != "") then {_npc action ["DropWeapon","GroundWeaponHolder" createVehicle getposATL _npc,secondaryWeapon _npc];};
...
UPSMON_GetIn_NearestStatic = {
	private["_vehicles","_npc","_units","_unitsIn","_grpid","_getin","_count","_spawninveh"];
	_grpid = _this select 0;	
	_npc = _this select 1;	
	_count = 0;
	_distance = 100;
	...
	//Solo tomamos las unidades vivas y que no estén en vehiculo
	{
		if ( (_x iskindof "Man") && _x == vehicle _x && alive _x && canmove _x && canstand _x) then {
			_unitsIn set [count _unitsIn, _x];			
		};
	}foreach _units;
	...
	if ( count _unitsIn > 0) then {		
		_units = [_grpid, _units, _vehicles, true,_spawninveh] call UPSMON_selectvehicles;				
	};
...
UPSMON_selectvehicles = {
	private["_vehicles","_emptypositions","_units","_unitsIn","_i","_grpid","_vehgrpid","_unit","_wp","_any","_index","_cargo","_spawninveh"];
	_grpid = _this select 0;	
	_units = _this select 1;
	_vehicles = _this select 2;	
	_any = _this select 3;	//meter en cualquier vehiculo
	_spawninveh = false;
	if ((count _this) > 4) then {_spawninveh = _this select 4;};
	...
	
  	{
		...
		if ( _vehgrpid == _grpid || (_emptypositions > 0 && _any)) then 
		{
			while {_i < _emptypositions && _i < count _units} do
			{
				...
				_unitsIn set [count _unitsIn, _unit];				
				_i = _i + 1;
			};
			...
			{			
				...
				[_grpid,_unitsIn,_vehicle,_spawninveh] spawn UPSMON_UnitsGetIn;	
...
UPSMON_UnitsGetIn = {
	...
	{		
		...
		{ 				
			...
			[_gunner,_vehicle,_spawninveh] spawn UPSMON_assignasgunner;					
...
UPSMON_doParadrop = {
	...
	_grp setVariable ["UPSMON_movetolanding",true];
	...
	{			
		...
		If (alive _npc) then 
		{
			_target = _npc findNearestEnemy _targetPos; 
	...
	if (!(surfaceIsWater (getposATL _heli)) && _paradrop) then { 	
		...
		{if (alive _x && canmove _x && leader _x != _x) exitwith { _x dofollow (leader _x);}}foreach _jumpers;
		_grp setVariable ["UPSMON_movetolanding",false];
		...
		{	
			If (_targetposred) then
			{
				_tries=0;
				_P = true;
				while { _P && (_tries<30)} do 
				{
					_tries=_tries+1;
					_targetPosTemp = [getposATL _heli,[0,100],[0,360],0,[0,100],1] call UPSMON_pos;
...
UPSMON_landHely = {
	...
	_landing = _heli getVariable ["UPSMON_landing",false];
	...
	_heli setVariable ["UPSMON_cargo", [], false];	
	_heli setVariable ["UPSMON_landing", false, false];
	(group _npc) setVariable ["UPSMON_movetolanding",false];	
...
UPSMON_nearestSoldiers = {
	...
	{			
		if (canmove _x && (getposATL _npc select 2) == (getposATL _x select 2)) then { _soldiers set [count _soldiers,_x];};
	}foreach _OCercanos;
