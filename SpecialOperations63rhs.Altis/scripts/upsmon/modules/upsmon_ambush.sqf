UPSMON_getAmbushpos = {
	...
	private ["_grp","_npc","_ambushdir","_ambushdist","_ambushType","_Mines","_npcdir","_positiontoambush","_diramb","_mineposition","_roads","_minetype1","_minetype2","_rdmdir","_gothit"];
...
UPSMON_SetAmbush = {
	...
	{
		_dist = 100;
		[_AmbushPosition,_positiontoambush,_dist,true,units _npc] call UPSMON_fnc_find_cover;
	};
...
UPSMON_FindAmbushPos = {
	...
	for "_i" from 1 to 50 do
	{
		...
		_los_ok = [_obspos1,_positiontoambush] call UPSMON_LOS;
		If (!surfaceIsWater _obspos1 && count (_obspos1 nearRoads 50) == 0 && _los_ok) then
		{_AmbushPositions set [(count _AmbushPositions),_obspos1];};
	};
