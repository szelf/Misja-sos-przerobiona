	...
	if ((_x select 0) == _template) then {
		...
		for [{_i=1},{_i<=_copies},{_i=_i+1}] do {
			...
			_c=-1;
			{
				_c=_c+1;
				if (_c>0) then {
					_newpos = _position findEmptyPosition [5, 200];
					sleep .4;
					if (count _newpos <= 0) then {_newpos = _position};
					_unittype= (_membertypes select _c) select 0;
					if (UPSMON_Debug>0) then {diag_log format["template %1 side %2 _unittype %3",_template,(_membertypes select _c),_unittype]};	
