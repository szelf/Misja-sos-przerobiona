...
AD_enabledVehicles = [ "B_Heli_Transport_01_F", "B_Heli_Transport_01_camo_F", "I_Heli_Transport_02_F", "RHS_CH_47F", "RHS_UH60M_MEV", "RHS_UH60M" ];
AD_condition 		= "((typeOf (vehicle player) in AD_enabledVehicles) && (speed (vehicle player) > -2) && (speed (vehicle player) < 2) && (getPos (vehicle player) select 2 < 2) && (driver (vehicle player) == player) && (!AD_running))";
AD_conditionChute	= "((typeOf (vehicle player) in AD_enabledVehicles) && (speed (vehicle player) >= 50) && (getPos (vehicle player) select 2 >= 75) && (driver (vehicle player) == player) && (!AD_running))";
