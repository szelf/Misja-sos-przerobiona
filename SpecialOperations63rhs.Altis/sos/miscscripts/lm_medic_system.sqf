...
lm_MS_const_bleedRegenAmount				= 0.014;
lm_MS_const_bleedRate						= 1.66;
lm_MS_const_internalDamageMultiplier		= 0.8;
lm_MS_const_internalDamageLimit				= 12.0;
...
lm_MS_const_wakeDelay						= 10.0;
lm_MS_const_wakeChance						= 0.05;
...
lm_MS_fnc_getInternalDamageDescription = {
	...
	if (_internalDamage <= 7.0) then {_description = "<t color='#FF0000'>heavily</t>"};
	if (_internalDamage <= 4.0) then {_description = "<t color='#FFAA00'>moderately</t>"};
...
lm_MS_fnc_reportCardiacArrestPlayer = {
	private ["_unit", "_isBreathing"];
	_unit = _this select 0;
	_isBreathing = _this select 1;
	if ((alive _unit) && (player distance _unit) <= 20.0) then {
		if (!_isBreathing) then {
			systemChat format ["%1 stopped breathing", (name _unit)];
		} else {
			systemChat format ["%1 started breathing again", (name _unit)];
		};
	};
};
...
lm_MS_fnc_doApplyPressureAction = {
	...
	if (!_isAnimCancelled && (alive player) && (alive _injured) && (_injured getVariable lm_MS_var_bleedTick == 0.0) && !(player getVariable lm_MS_var_isIncapacitated) && (_injured getVariable lm_MS_var_isIncapacitated) && !(_injured getVariable lm_MS_var_isBeingDragged)) then {
		if (_wasBleeding) then {
			hint format ["%1 stopped bleeding", (name _injured)];
		} else {
			hint format ["%1 is not bleeding", (name _injured)];
		};			
	};
};
...
lm_MS_fnc_doReviveAction = {
		...
		lm_MS_animHandle = [_injured] spawn {
			...
			while {true} do {			
				...
				if (_internalDamage <= 0.0) then {
					...
					_bleedAmount = 0.4 min _bleedAmount;
					...
					hint format ["%1 was revived", (name _injured)];
				};
				if (lm_MS_isDebug) then {
	...
	if (!_isAnimCancelled && (alive player) && (alive _injured) && !(_injured getVariable lm_MS_var_isBreathing) && !(player getVariable lm_MS_var_isIncapacitated) && (_injured getVariable lm_MS_var_isIncapacitated) && !(_injured getVariable lm_MS_var_isBeingDragged)) then {
		if (_wasBreathing) then {
			hint format ["%1 stopped breathing", (name _injured)];
		} else {
			hint format ["%1 is not breathing", (name _injured)];
		};			
	};
};
		...
		if ((alive player) && (alive _injured) && !(player getVariable lm_MS_var_isIncapacitated) && (_injured getVariable lm_MS_var_isIncapacitated) && !(_injured getVariable lm_MS_var_isBeingDragged)) then {
			if !(_injured getVariable lm_MS_var_isBreathing) then {
				...
				if (_rand <= _chance) then {
					...
					[[_injured], "lm_MS_fnc_reportCardiacArrestPlayer", true, false] call BIS_fnc_MP;
					hint format ["%1 started breathing again", (name _injured)];
				} else {
					hint format ["CPR on %1 failed", (name _injured)];
				};
			} else {
				_internalDamage = _injured getVariable lm_MS_var_internalDamage;
...
lm_MS_fnc_addMedicActions = {
	...
	if (_unit == player) then {
		_unit addAction ["<t color='#FFFF00'>Check myself</t>", {
...
lm_MS_fnc_applyInternalDamage = {
	...
	switch (_selectionName) do {
		case lm_MS_const_bodyPartHead 	: {_scalar = 6.0};
...
lm_MS_fnc_applyDamageModifier = {
	private ["_selectionName", "_damage"];
	_selectionName 	= _this select 0;
	_damage 		= _this select 1;
	
	if (_selectionName == lm_MS_const_bodyPartHead) then {
		if ((headgear player in lm_MS_const_helmetList) && (random 1.0 <= 0.8)) then {
			if (_damage > 0.1) then {
				...
				playSound3D [_hitSound, player, false, getPosASL player, 20.0, 1.0, 10.0];
			};
		};
	};
...
lm_MS_fnc_applyCardiacArrest = {
	if (player getVariable lm_MS_var_isBreathing) then {
		...
		[[player], "lm_MS_fnc_reportCardiacArrestPlayer", true, false] call BIS_fnc_MP;
	...
	if !(player getVariable lm_MS_var_isBleeding) then {	
		...
		[] spawn {
			while {true} do {
				...
				if ((!_isIncap && (damage player <= 0.25)) || (_isIncap && ((player getVariable lm_MS_var_bleedTick) <= 0.0)) || !alive player) exitWith {
				...
				if (_bleedAmount >= 1.0) then {
					...
					if (!_isIncap && (_bleedAmount >= 0.5)) then {
...
lm_MS_fnc_handleDamage = {
	...
	if ((_unit == player) && (alive player)) then {	
		...
		_damage = [_selectionName, _damage] call lm_MS_fnc_applyDamageModifier;
		...
		if !(isNull _source) then {
			...
			if ((_selectionName == lm_MS_const_bodyPartHands) && (random 1 <= 0.05)) then {
...
lm_MS_fnc_updatePlayerVars = {
	...
	_isBreathing 		= _this select 5;
	_isIncapacitated 	= _this select 6;
	_isTreating 		= _this select 7;
	_isBeingTreated 	= _this select 8;
	_isBeingDragged 	= _this select 9;
