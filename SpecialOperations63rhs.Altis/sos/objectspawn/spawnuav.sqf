/*
	Use this script to allow specific classes to spawn UAVs.
...
if ((typeOf _caller) in OS_allowUAVSpawnWest) then {
...
//			_veh = _vehicleType createVehicle [_spawnPos select 0, _spawnPos select 1, (_spawnPos select 2) + 0.5];
            _veh = createVehicle [ _vehicleType, [_spawnPos select 0, _spawnPos select 1, (_spawnPos select 2) + 0.5],[],0,"NONE"];
