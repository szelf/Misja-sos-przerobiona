...
// Check if players class is allowed to spawn a vehicle. Everyone is allowed to spawn a quad bike.
if (((typeOf _caller) in OS_allowVehicleSpawnWest) || (_vehicleType == "B_Quadbike_01_F")) then {
	...
	if (!_whitelisted || (_whitelisted && ((getPlayerUID _caller) in SOS_whitelistedPlayers))) then {
		...
		hintSilent "You can't spawn this vehicle, if you are not whitelisted!";
		playSound "warning1";
	};
} else {
